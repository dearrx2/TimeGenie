...
class requestSick extends StatefulWidget {
  const requestSick({Key? key}) : super(key: key);

  @override
  State<requestSick> createState() => _requestSickState();
}

class _requestSickState extends State<requestSick> {
  ...
  Future<List<DocumentSnapshot>> getSickDatesByStatus(
    ...
    QuerySnapshot snapshot = await requestSickCollection
  ...
  Future<void> _selectDateRange(bool isStartDate) async {
    ...
    // Call getSickDatesByStatus to retrieve disabled dates for 'approved' and 'waiting' statuses
    List<DocumentSnapshot> approvedDatesSnapshots =
    await getSickDatesByStatus('approved', user.uid);
    List<DocumentSnapshot> waitingDatesSnapshots =
    await getSickDatesByStatus('waiting', user.uid);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text(
            MyLocalizations.translate("text_SickLeave"),
            style: appBar,
          ),
                      ...
                      children: [
                        Text(
                          MyLocalizations.translate(
                              "text_UploadImage(Optional)"),
                          style: const TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
                          ),
                        ),
                        if (media == null) // Show the button only when media is null
                          Padding(
                                      ...
                                      children: [
                                        Text(
                                          media!.path.split('/').last.length >
                                              30
                                              ? '${media!.path.split('/').last.substring(0, 27)}...'
                  ...
                  Navigator.pushReplacement(
                    ...
                    MaterialPageRoute(
                      builder: (context) => SuccessPageRange(
                        icon: 'assets/images/icon_tag_leave/sick_tag.png',
